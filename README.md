...
Set a name for your project (login-app) and the rest leave it as defaults.
...
1. remove all files from `public/*`
2. Clear `src/app/globals.css` except @import "tailwindcss";
3. clear `src/app/page.ts` and replace with:
...
1. stop dev server and install [Better Auth](https://www.better-auth.com/docs/installation)
...
9. A `prisma` folder must be created in your root folder, open the `schema.prisma` file inside
10. create **Post** Model inside `schema.prisma`
...
12. adjust **scripts** in `package.json`, this will keep our web server sync with our prisma schema
...
13. create single Prisma Client in `lib/prisma.ts`
...
15. setup prisma adapter with better-auth, go to `src/lib/auth.ts` update the file to
...
- `Account`
- `Verification`
  Go to ``prisma/schema.prisma and paste the tables.
...
Delete `auth.schema.prisma` file.
Push database changes
...
21. Open Neon dashboard and go tot tables, verify your tables were created.
22. create Mount Handler file in `app/api/auth/[...all]/`, name your file `route.ts`
...
23. create Client instance in `lib/auth-client.ts`
...
25. Create Sign Up Page PT1
    create a `page.tsx` file inside `app/auth/register/` ass these lines:
...
26. Create a register form.
    create `register-form.tsx` file inside `src/components/ui/` and add these lines:
...
27. setup Toaster, got to `src/app/` and open `layout.tsx` file and add the Toaster component.
...
28. Go back to `src/components/ui/register-form.tsx` and add toasts to the form
...
export const RegisterForm = () => {
  ...
  async function handleSubmit(evt: React.FormEvent<HTMLFormElement>) {
    evt.preventDefault(); /********************   Setup Toast   *************************/
...
29. setup sign-up procedure
    go to `lib/auth-client.ts` and export the signUp function.
...
Update `src/components/ui/register-form.tsx` and add imports at the top and these lines below toast:
...
30. Modify better auth password length property
    got to `lib/auth.ts`, under emailAndPassword add the minPasswordLength property:
...
31. Display your session on a Profile page
    In the `/app` folder create a new folder called `/profile`, inside the folder create a `page.tsx` file.
  ...
  return (
    <div className="px-8 py-16 container mx-auto max-w-screen-lg space-y-8">
      ...
          <pre className="text-sm overflow-clip">
                        {JSON.stringify(session, null, 2)} //Display session
        attributes            {" "}
      </pre>       {" "}
    </div>
  );
...
32. Create a sign out button.
    go to `src/components/` create `sign-out-button.tsx` file
...
33. Create a login page
    go to `src/app/auth/` create a `/login` folder and a `page.tsx` file within it.
...
export default function Page() {
  return (
    <div className="px-8 py-16 container mx-auto max-w-screen-lg space-y-8">
           {" "}
      <div className="space-y-8">
                <ReturnButton href="/" label="Back to Home" />        <h1 className="text-3xl font-bold">
          Login
        </h1>     {" "}
      </div>
            <LoginForm />     {" "}
...
go to `src/components` and create `login-form.tsx` file
...
go to `lib/auth-client.ts` and export the sing in function
